...
\noindent\textit{Background reading before starting Phase~1:} review
\S\ref{sec:background} (ClawHDF5 crate layout), \S\ref{sec:merkle} (tree
construction and domain-separation rules), and \S\ref{sec:merkle-storage}
(hybrid attribute~$+$~companion-dataset layout).
...
\noindent\textit{Background reading before starting Phase~2:} review
\S\ref{sec:pq-hybrid} (hybrid signature construction), \S\ref{sec:filter-order}
(Encrypt-then-MAC ordering), \S\ref{sec:nonce} (nonce derivation), and
\S\ref{sec:security-analysis} (game-based argument structure).
