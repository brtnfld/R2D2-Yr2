...
The integrity of these datasets is increasingly critical not only for archival
fidelity but for downstream \emph{machine-learning pipelines}: when HDF5 files
serve as training corpora, even small-scale data poisoning---corrupting as little
as 0.1\,\% of training samples---has been shown to induce error-rate increases
exceeding 80\,\%~\cite{neurips2020poisoning}. Cryptographic authentication
enables consumers to detect and localize such corruption before it propagates
into trained models or irreproducible simulation results.
...
\begin{itemize}[nosep]
  ...
  \item \texttt{clawhdf5-filters}: Filter pipeline (deflate backends)
...
its filter pipeline---\texttt{clawhdf5-filters} provides deflate backends
and \texttt{clawhdf5-format} implements shuffle and fletcher32. Cryptographic operations will therefore be
...
HDF5 uses background threads, achieving significant latency hiding for
...
Tamper detection and data integrity are deep, mature research areas spanning
...
The need for a cryptographic integrity primitive is driven both by operational
security concerns and by the \emph{FAIR data principles}~\cite{wilkinson2016fair}
(Findable, Accessible, Interoperable, Reusable). The Reusable pillar requires
that data are ``associated with detailed provenance,'' and the Accessible
pillar implies that a recipient of a partial dataset delivery must be able to
verify authenticity without redownloading the whole. Current HDF5 practice
satisfies neither demand at terabyte scale; this survey identifies which, if
any, existing primitives can.
...
preserving the checksum. Adler-32 and Fletcher-32 are linear over a small
modulus ($65521$ and $2^{16}-1$ respectively), which similarly admits a
...
schemes~\cite{boneh2011homomorphicmac} for network coding) extend this family
by allowing intermediate nodes to aggregate keyed authenticators; verification
still requires the secret key.  Homomorphic hashing (e.g., Meta's
LtHash~\cite{lthash}) is conceptually adjacent but distinct: LtHash is an
\emph{unkeyed} hash function whose outputs lie in a group with a homomorphic
addition law, relying on the hardness of lattice-style problems rather than a
shared secret.  Crucially, LtHash \emph{does not require a secret key to
verify}---any party can recompute and check the hash---which sets it apart from
MACs in the verification model.  Both constructions allow intermediate nodes to
incrementally aggregate authenticators when combining data packets or
propagating chunk updates, avoiding the need to re-authenticate from
scratch---a property relevant to streaming and in-transit integrity.
...
cannot verify without it. This directly conflicts with the FAIR Reusable
pillar~\cite{wilkinson2016fair}, which requires that provenance be verifiable
by any party, not only the original data producer. Symmetric authentication
is therefore appropriate as a \emph{component} (e.g., AEAD ciphertext
authenticity) but cannot serve as the public provenance primitive on its own. Encrypt-then-MAC composition is the
...
\cite{rfc8032} provides non-repudiable signatures with public verifiability:
...
Trillian~\cite{trillian} applies the same model to software-supply-chain
...
\noindent\textbf{Overlay Merkle tree vs.\ in-format authenticated B-trees.}
HDF5 uses internal B-trees extensively to track chunk allocations within the
file (the Version~1 B-tree in the HDF5 specification manages chunk-storage
indices for each dataset). Tamassia's authenticated data structure framework
applies directly to B-trees, raising the question of whether an authenticated
B-tree built on HDF5's existing chunk index would subsume the need for an
external Merkle overlay. Three factors rule this out for our setting.
First, HDF5's internal B-trees are highly mutable, deeply coupled to the
library's metadata-management code, and among the most complex and brittle
sections of the C~HDF5 library; augmenting their node structure to carry hash
accumulators would require invasive changes to the lowest-level format layer
and would break forward compatibility with existing readers. Second, the B-tree
topology is determined by the HDF5 storage allocator, not by the logical chunk
grid, so leaf positions do not align with chunk boundaries in a way that admits
clean hyperslab-aligned proofs. Third, a companion-dataset overlay is
format-agnostic: it can be constructed without modifying the library,
is embarrassingly parallel across MPI ranks (each rank hashes its own chunk
subset independently, and results are reduced via $O(\log P)$ collective
operations), and carries no format-compatibility risk for existing HDF5 files.
...
STARKs~\cite{bensasson2018stark} and are post-quantum and require no trusted
setup; however, their prover overhead scales with circuit size and their proof
sizes run 10--100\,KB, making them uncompetitive with simple Merkle inclusion
proofs for static data verification. All pairing-based schemes (KZG, Verkle,
Groth16~\cite{groth2016sn}) are broken by Shor; all require an $O(N)$
multi-scalar multiplication to commit; none are competitive with hash trees for
our setting at terabyte scale.
...
a controlled platform but relies on recurrent vulnerability disclosures,
...
Both ZFS and Btrfs use \emph{copy-on-write} (COW) semantics: every mutation
writes a new block rather than overwriting the original, producing an implicit
sequence of immutable snapshots that enables point-in-time rollback and
self-healing from redundant copies. COW is valuable for recoverability, but
it does not by itself establish the \emph{authenticity} of any given snapshot:
an adversary with write access can produce a COW snapshot of tampered data.
...
format rather than reside exclusively in the storage substrate---precisely
the FAIR Interoperability principle~\cite{wilkinson2016fair}, which requires
that data and its associated metadata remain usable across platforms and
institutions without depending on any particular infrastructure.
...
Emerging \emph{virtualized} formats---Kerchunk~\cite{kerchunk} and its
successor VirtualiZarr---map chunk coordinates to byte ranges inside legacy
HDF5/NetCDF files stored on S3-compatible object storage, enabling cloud-native
access to archival data without format conversion; however, they inherit the
integrity limitations of the underlying objects and add no cryptographic
verification layer of their own. Modern array engines such as
TileDB~\cite{tiledb2020} provide high-performance chunked storage for
multi-dimensional arrays in genomics and geospatial workloads and support
at-rest AES-256-GCM encryption at the tile level, but rely on the underlying
cloud store for object-level integrity and provide no hierarchical
verifiable-subset capability---a verifier wishing to confirm the integrity of
an arbitrary sub-array must re-read and re-authenticate the entire relevant
tile set.
...
\subsection{Statistical and machine-learning-based anomaly detection}
\label{sec:ml-detection}

A complementary detection strategy employs statistical models or machine
learning to identify anomalous patterns in dataset contents without relying
on provenance metadata. Anomaly-based detectors applied to ML training
corpora have achieved reported detection accuracy of 75--95\,\% for targeted
data-poisoning attacks~\cite{neurips2020poisoning}. Quantum machine learning
(QML) variants have also been proposed for anomaly detection in
high-dimensional scientific arrays, exploiting quantum amplitude estimation
to accelerate the inner-product computations underlying nearest-neighbor
outlier scoring.

In forensic settings, NTFS journal artifacts---specifically the Update
Sequence Number journal (\texttt{\$UsnJrnl}) and the transactional log
(\texttt{\$LogFile})---record every filesystem write at the volume level,
enabling post-incident reconstruction of which objects were modified and by
which process. These host-based forensic signals are complementary to
file-format-level integrity: they provide post-hoc attribution evidence
rather than preventing modification or enabling a recipient to verify
integrity without trusting the host operating system.

A related design goal is \emph{tamper tolerance}---keeping a system
operational despite active attacks---as opposed to pure detection.
Tamper-tolerant software (TTS)~\cite{viticchie2016tts} employs obfuscation,
self-checking guards, and graceful-degradation modes so that a system
continues operating even when portions of its code or data have been
modified. Periodic consistency checks compare pre-computed values against
runtime state; on detecting a mismatch the system may crash, degrade, or
alert. The fundamental limitation of all such approaches is that there are
no \emph{provably secure} software anti-tampering methods: a proof of
tamper-resistance would require a formal model capturing all attack surfaces
(impossible for physical systems), a reduction to an unsolved hard problem,
and that reduction to hold even when the attacker controls the
hardware~\cite{viticchie2016tts}.

These mechanisms are complementary to but cannot substitute for
cryptographic authentication. An anomaly detector signals that data
\emph{appears} unusual but cannot prove \emph{who signed it} or bind a
cryptographic commitment to a specific byte sequence. A TTS system
tolerates runtime code tampering but does not authenticate stored data.
Forensic journal analysis provides post-hoc attribution but cannot prevent
unauthorized writes or offer public verifiability. None of these mechanisms
provides sub-dataset partial verification, public verifiability without a
shared secret, or post-quantum durability---the requirements that govern the
selection in \S\ref{sec:selection}. They are therefore not candidates for
the primary integrity primitive but remain useful complementary signals in a
...
filesystem/HPC storage mechanisms (not portable), and ML-based anomaly
detection and tamper-tolerant software (probabilistic, not cryptographically
binding, no public verifiability). \S\ref{sec:selection}
...
Before deriving the requirements, we define the adversary model
...
\begin{description}[nosep]
  ...
  \item[T6: Security stripping and algorithm downgrade.]
    ...
    downgrading the hash from \texttt{blake3} to \texttt{sha256-truncated-128}, or
...
We translate the problem statement of \S\ref{sec:intro} into twelve concrete
requirements that any candidate primitive must meet for the HDF5 setting.
Several requirements correspond directly to FAIR data
principles~\cite{wilkinson2016fair}: R1 and R5b realize the \emph{Accessible}
pillar (verifiable partial delivery without a full re-download); R6 realizes
\emph{Reusable} (any downstream collaborator can verify provenance using only
a public key); and R8--R9 ensure the provenance guarantee is durable across
the multi-decade archival horizon that FAIR envisions for scientific data.
...
\begin{description}[nosep]
  ...
  \item[R3: In-place chunk update.]
    ...
    $k$ must cost $O(\log N)$ in \emph{tree update} work (recomputing the
    leaf hash and the $\lceil\log_2 N\rceil$ ancestor nodes on the path to
    the root) and $O(\log N)$ in signing-plane work, not $O(N)$.  The
    physical I/O cost is governed separately by the filesystem or object store
    and may involve copy-on-write semantics; this requirement addresses only
    the cryptographic maintenance overhead.
  ...
  \item[R11: Distributed parallel construction.]
    In HPC workflows, $P$ compute nodes write disjoint chunk subsets to the
    same dataset concurrently via MPI-IO. Leaf-level hash computation must be
    \emph{embarrassingly parallel}: each node independently hashes its own
    chunks, and the tree must be reducible via $O(\log P)$ rounds of MPI
    collective operations with no global lock on the commitment structure.
    Primitives that require a sequential multi-scalar multiplication (MSM)
    across all $N$ chunks to commit or update---RSA accumulators, KZG
    polynomial commitments, ZK-SNARK circuits---are disqualified by this
    requirement as decisively as by R10.
  \item[R12: Structure confidentiality for sparse encrypted datasets.]
    For encrypted datasets, the integrity structure must not reveal the
    physical sparsity pattern (allocation grid) of the underlying data to an
    observer who cannot decrypt the payload chunks. Equivalently: an
    authenticated observer who knows which leaves are null vs.\ populated
    gains information about dataset geometry (e.g., geographic boundaries in
    sparse climate data, non-zero regions in genomic arrays) independent of
    the payload. The construction must either hide null-leaf positions or
    provide a mechanism---using a key distinct from the payload DEK---by which
    authorized auditors can distinguish null from populated regions without
    decrypting chunk content.
\end{description}

\subsection{Comparison of candidate primitives}

Table~\ref{tab:integrity-comparison} maps the seven alternative classes plus
Merkle tree (eight columns total) identified in the survey against these twelve
requirements.
...
R5b coverage proof      & $\times$   & $\times$   & manifest$^f$ & complex    & \checkmark & \checkmark & $\times$   & \checkmark$^k$ \\
...
R10 HPC-feasible build  & \checkmark & \checkmark & 3\,h$^h$   & 3\,h$^a$   & 30\,s$^a$  & $>$24\,h$^b$ & \checkmark & 30\,s$^i$ \\
R11 distrib.\ parallel  & \checkmark & \checkmark & \checkmark & $\times$   & $\times$   & $\times$   & vendor     & \checkmark \\
R12 struct.\ confid.    & $\times$   & $\times$   & $\times$   & $\times$   & $\times$   & $\times$   & $\times$   & opt-in$^j$ \\
\bottomrule
\end{tabular}}
\caption{Candidate primitives against the twelve requirements of
...
$^a$~RSA accumulator: each update is one $O(N)$ modular exponentiation,
$\sim$30\,min--3\,h on commodity HW; KZG: one $O(N)$ multi-scalar multiplication
over BLS12-381, $\sim$30\,s--30\,min on commodity HW (Pippenger, single core).
$^k$~Merkle R5b: coverage proof requires the signed grid-hash~$+$~Merkle-path
coverage certificate (\S\ref{sec:subset}), not raw inclusion proofs alone.
...
aggregate via a leadership-class parallel filesystem, e.g., DAOS on Aurora or
Lustre on Frontier, for 10\,TB);
tree aggregation over pre-computed leaf hashes adds $<1$\,s on a single core.
$^j$~Merkle R12: structure confidentiality is \emph{opt-in} via
PRF-masked null hashes ($H(\texttt{0x02}\,\|\,\text{PRF}(SK,\texttt{"null"}))$,
where $SK$ is a Structure Key derived from the KEK); see \S\ref{sec:merkle-storage}.
No other candidate provides even an opt-in path to structure hiding.
...
R1--R12 \emph{without trusted setup or post-quantum liability}:
...
is the only primitive achieving R1--R12 \emph{simultaneously, without a
...
\begin{description}[nosep]
  \item[Most scientific data is unencrypted.]
    ...
    data, and genomic reference sequences are public or community-shared---
    canonical FAIR datasets~\cite{wilkinson2016fair} that must be Accessible
    and Reusable by any downstream researcher. These datasets have no
    confidentiality requirement, but they critically need tamper
    detection---a corrupted simulation checkpoint or a silently modified
...
\smallskip
{\scriptsize\resizebox{\linewidth}{!}{%
...
\noindent\textbf{Space-filling curve linearization.}
A promising direction for closing this gap is to replace row-major order with a
\textbf{space-filling curve} (SFC) to map the $r$-dimensional chunk grid to a 1D
leaf sequence.  Morton coding (Z-order curve)~\cite{morton1966} is particularly
attractive: it is computed in $O(1)$ per chunk via bit-interleaving of the chunk
coordinates, requires no additional metadata, and has been shown to preserve
multi-dimensional locality in a provably hierarchical fashion---neighboring
chunks in $r$-dimensional space map to nearby positions in the 1D Morton order.
A Hilbert curve offers asymptotically better locality constants but requires a
more involved coordinate transform.  Under Morton or Hilbert ordering, a
contiguous 3D hyperslab corresponds to a union of \emph{short contiguous
ranges} of leaves, substantially increasing the fraction of shared internal
Merkle nodes eligible for deduplication and pushing empirical proof sizes
toward the theoretical $O(\mathrm{SurfaceArea}\times\log N)$ bound.
Quantifying this improvement---across chunk shapes, hyperslab geometries, and
dimensionalities---is a central goal of RQ6 and would make for a compelling
...
or the dataset must be allocated as fully dense.
However, using a DEK-derived PRF introduces a \textbf{verification
trade-off}: because a third-party auditor lacks the DEK, they cannot
compute the PRF-masked null hashes and therefore cannot verify the Merkle
tree's structure for sparse regions---they cannot even confirm which leaves
are null versus populated without decrypting.  To permit auditable
verification without exposing chunk plaintexts, the design can distribute a
separate \textbf{Structure Key} $SK = \text{KDF}(\text{KEK},\texttt{"structure"})$
derived from the Key Encryption Key rather than the Data Encryption Key.
Auditors receive $SK$, which lets them compute
$H(\texttt{0x02}\,\|\,\text{PRF}(SK,\texttt{"null"}))$
and reconstruct the full tree topology without ever obtaining the DEK or
decrypting any payload chunk.  The choice between DEK-derived and
KEK-derived PRF nulls is a policy decision: DEK-derived maximizes
confidentiality at the cost of excluding third-party auditors; SK-derived
permits structural audits while still concealing chunk content.
This trade-off is noted as a configuration concern in the security design document.
The Structure Key mechanism is scoped as a design option within Phase~2
(\S\ref{sec:impl}) alongside the choice of null-hash PRF derivation path.
...
\noindent\textbf{Write amplification.}
Because the companion dataset's 4\,KB chunks each hold $4096/32 = 128$ tree
nodes, updating a \emph{single} data chunk requires rewriting its leaf hash
plus the $O(\log N)$ ancestor nodes on the path to the root.  If those
ancestors span multiple 4\,KB companion chunks (which occurs as soon as the
tree depth exceeds one chunk boundary), a single data write triggers
$O(\log N / \log 128) = O(\log N / 7)$ companion-chunk I/O operations in
addition to the data chunk write itself.  Under concurrent or parallel
writers---common in HPC checkpoint workloads---multiple independent data
updates will frequently invalidate the \emph{same} upper-level companion
chunks (which hold shared ancestor nodes near the root), producing contention
and redundant rewrite traffic.  The Map-Reduce aggregator (\S\ref{sec:impl})
amortizes this by batching leaf updates before recomputing the path to the
root, but the residual I/O amplification must still be characterized
experimentally; this is explicitly scoped within RQ3.
...
\noindent\textbf{Strict Mode bootstrapping.}
A subtlety in Strict Mode is the \emph{bootstrapping problem}: the mechanism
by which a verifier knows a file is policy-governed must itself be
tamper-evident.  If policy membership is stored solely inside the HDF5 file
(e.g., as an attribute), an attacker who can strip the Merkle root can equally
strip the policy flag, causing the verifier to silently fall back to
non-strict mode.  Strict Mode therefore requires an \textbf{out-of-band
policy registry}---a separate, protected source of truth that maps file
identifiers (e.g., cryptographic file fingerprints or storage URIs) to their
required policy level.  Concrete mechanisms include: filesystem extended
attributes on the directory or mount point (xattr enforced by a MAC policy,
e.g., SELinux or AppArmor); a site-managed policy server queried at file
open; or an LDAP/SCIM directory entry for the dataset.  The verifier consults
this registry \emph{before} attempting to read the file, so policy enforcement
is independent of file content.  If the registry is unavailable---for
example, when a file is transferred to a disconnected environment---the
verifier must either fail closed (reject all files) or fall back to an
embedded policy hint, accepting the reduced security guarantee.  The C~HDF5
design document (\S\ref{sec:impl}, Phase~2) will specify the policy
distribution interface and the required trust properties of the registry.
...
per (key, chunk, version) triple.

\noindent\textbf{Crash-safe version counter increments (WAL requirement).}
A subtle conflict exists between the crash-consistency write order of
\S\ref{sec:merkle-storage} (``write chunk first, then update companion
tree'') and the nonce-safety requirement above (``$v$ must be incremented
\emph{before} the chunk is encrypted and written''). If $v$ is incremented
only in the companion dataset after the chunk write, a crash between the
chunk write and the companion update leaves $v$ at its old value; the next
write to the same chunk would then re-derive the same nonce, catastrophically
reusing the keystream. Conversely, incrementing $v$ in the companion dataset
\emph{before} the chunk write violates the crash-consistency write order.

The resolution is a \textbf{Write-Ahead Log (WAL)} for version counters:
\begin{enumerate}[nosep]
  \item Append a journal record $(\mathit{chunk\_idx},\; v_\text{new})$ to a
    crash-durable WAL (e.g., a small dedicated HDF5 dataset or an OS-level
    append-only file) \emph{before} computing the nonce.
  \item Derive the nonce using $v_\text{new}$, encrypt the chunk, and write
    the ciphertext.
  \item Promote the journal record into the companion dataset (update the
    leaf's stored version counter to $v_\text{new}$) and recompute the
    Merkle path to the root.
  \item Mark the journal record as committed.
\end{enumerate}
On crash recovery, any uncommitted journal record indicates an in-progress
write: the system replays the chunk encryption (using the journaled
$v_\text{new}$) and completes the companion update before declaring the
file consistent. This guarantees the nonce is never reused regardless of
crash timing. The C~HDF5 design document (\S\ref{sec:impl}, Phase~2)
will specify the exact WAL format and interaction with HDF5 flush semantics. The version counter also serves double duty
...
\begin{aligned}
  ...
  R &= \text{root} \,\|\, \text{companion\_hash} \,\|\,
       v_\text{dataset} \,\|\, \tau \,\|\, \text{AlgID}
\end{aligned}
\end{equation}
...
to $\sim$50~KB depending on parameters; SLH-DSA-128s is $\sim$7.7~KB,
SLH-DSA-256f reaches $\sim$49~KB). Even with a single file-level
...
will reject it.
\textbf{Length-prefixing requirement.}  If compression is applied before
encryption (as in the shuffle$\to$compress$\to$encrypt pipeline of
\S\ref{sec:filter-order}), the ciphertext length is variable across chunks.
Concatenating variable-length fields without explicit length delimiters
can admit \emph{boundary-shifting} (a.k.a.\ length-extension at the
field boundary): a transcript $(\text{Ct}_1 \| \text{Tag}_1)$ with
$|\text{Ct}_1| = n_1$ and $(\text{Ct}_2 \| \text{Tag}_2)$ with
$|\text{Ct}_2| = n_2$ may produce the same bit string if a suffix of
$\text{Ct}_1$ is reassigned to the front of the tag field.  The
implementation must therefore encode the serialization as
$\texttt{0x00} \| \text{len}(k) \| k \| \text{len}(\text{Ciphertext}_k) \|
\text{Ciphertext}_k \| \text{Tag}_k \| v_k$, where all length fields are
fixed-width big-endian integers.  Because the Poly1305 tag is always exactly
16\,bytes and the version counter $v_k$ is always a fixed-width integer, only
the ciphertext length field needs to be explicit; nonetheless, including all
length fields is the safest practice and is mandated in the implementation
specification. This is especially important under Sparse Merkle Trees,
...
$H(\texttt{0x00} \,\|\, \text{len}(k) \,\|\, k \,\|\, \text{len}(\text{Chunk}_k) \,\|\,
\text{Chunk}_k \,\|\, v_k)$;
the same length-prefixing rules from above apply whenever the chunk payload
may be variable-length (e.g., after compression is applied in the filter pipeline).
...
\begin{description}[nosep]
  \item[Game-based argument (baseline, in scope).]
    ...
    (EtM filter pipeline) $+$ (version-counter nonce) $+$ (hybrid
...
\begin{enumerate}[label=\textbf{RQ\arabic*}]
  ...
  \item How does incremental tree extension perform for append-heavy workloads
    ...
    dataset?  Additionally, what is the write-amplification factor---number of
    companion-dataset chunk I/Os per data chunk update---as a function of tree
    depth, companion chunk size, and writer concurrency, and does the
    Map-Reduce aggregator reduce amplification to an acceptable level for
    parallel HPC writers?
...
This section describes the work as concrete, step-by-step tasks ordered to
build progressively from a working ClawHDF5 environment through the three
primary research contributions. Each task specifies what to build, which Rust
crate to work in, how to test the result, and a clear \emph{Done~when}
acceptance criterion. Items marked \textbf{(Move~$N$)} are publication-unit
contributions targeted at the resulting paper (Move~1 = verifiable subset
...
evaluation). AES-256-CTR~$+$~HMAC-SHA-256 filters and live Sigstore service
integration are deferred to Phase~3 to preserve focus on the critical path.

%% ── Phase 1 ──────────────────────────────────────────────────────────────────
\subsection{Phase 1: Core Architecture and Verifiable Subset (Months 1--4)}

\noindent\textit{Background reading before starting Phase~1:} read all of
\S\ref{sec:background} (HDF5 data model, chunked storage, S2-D2 framework
architecture, and ClawHDF5 crate layout), then all of \S\ref{sec:merkle}
(the full Merkle-Tree Provenance design: threat model, why integrity is
independent of encryption, tree construction, hash algorithm selection,
verifiable subset extraction, storage layout, and signing), with particular
attention to \S\ref{sec:merkle-storage} (hybrid attribute~$+$~companion-dataset
layout and domain-separation prefixes \texttt{0x00}/\texttt{0x01}/\texttt{0x02}).

%% ── P1.1 ──────────────────────────────────────────────────────────────────
\paragraph{P1.1\enspace Platform evaluation / go--no-go gate (Weeks~1--2).}
Before writing any Merkle-tree code, confirm that ClawHDF5 supports all the
I/O patterns this work requires. If any step below fails, activate the
standalone-library fallback described in~\S\ref{sec:risks}.

\begin{enumerate}[nosep]
  \item Clone the ClawHDF5 repository and run \texttt{cargo test --workspace}.
    All existing tests must pass with no warnings about missing features.
  \item Generate a synthetic 10\,GB HDF5 file that has at least three datasets
    with different chunk sizes (e.g., 64\,KB, 256\,KB, 1\,MB). Use
    ClawHDF5's high-level API in \texttt{clawhdf5}.
  \item Write a test that opens the file, iterates over every chunk of one
    dataset (reading raw chunk bytes), modifies one chunk in memory, and
    writes it back. Re-open the file and read the modified chunk to confirm
    the write succeeded without corrupting adjacent chunks.
  \item Repeat step~3 on a real NOAA sample file from the provided test-data
    archive.
  \item \emph{Done when:} All four steps above complete without panics or
    data-corruption failures and the modified file re-opens cleanly.
\end{enumerate}

%% ── P1.2 ──────────────────────────────────────────────────────────────────
\paragraph{P1.2\enspace \texttt{MerkleTree} struct and hash benchmarks
(Weeks~2--5).}
Implement the core tree data structure and compare three hash functions across
HDF5-typical chunk sizes (RQ4). Read \S\ref{sec:hash-select} for the
performance--compliance tradeoffs among SHA-256, BLAKE3, and K12.

\begin{enumerate}[nosep]
  \item In \texttt{clawhdf5-format/src/merkle.rs}, add:
\begin{lstlisting}
pub enum HashAlg { Sha256, Blake3, K12 }

pub struct MerkleTree {
    pub leaves:     Vec<[u8; 32]>,  // leaf hashes, left to right
    pub nodes:      Vec<[u8; 32]>,  // complete binary tree, level-order
    pub root:       [u8; 32],
    pub alg:        HashAlg,
    pub padded_n:   usize,          // next power of 2 >= leaves.len()
    pub actual_n:   usize,          // true leaf count (chunks in dataset)
}
\end{lstlisting}
  \item Implement \texttt{MerkleTree::build(leaves: \&[[u8;32]], alg: HashAlg)
    -> Self}. \textbf{Tree shape:} scientific datasets rarely have
    power-of-2 chunk counts. Pad the leaf array with the computed null
    sentinel $H(\texttt{0x02}\,\|\,\texttt{null})$ (as defined in
    \S\ref{sec:merkle-storage}) up to the next power
    of~2 before building. This keeps the level-order index arithmetic
    ($\text{left} = 2i+1$, $\text{right} = 2i+2$) correct for all~$N$.
    Store the actual leaf count in \texttt{actual\_n} and the padded count
    in \texttt{padded\_n}; proof paths must use \texttt{padded\_n} for
    index arithmetic but must not deliver null-padded leaves to the verifier.
    \textbf{Maximum depth:} enforce $\lceil\log_2 \mathtt{padded\_n}\rceil
    \leq 40$ (supporting up to $2^{40} \approx 1$\,Trillion chunks);
    return \texttt{Err(MerkleError::TreeTooDeep)} for larger inputs to
    prevent out-of-memory attacks (threat~T7 in \S\ref{sec:threat}).
    Apply domain-separation prefixes exactly as specified in
    \S\ref{sec:merkle-storage}: prepend \texttt{0x00} before hashing leaf
    inputs, \texttt{0x01} before hashing pairs of internal nodes, and
    \texttt{0x02} for unallocated sparse-chunk slots.
  \item Add a helper \texttt{hash\_chunk(data: \&[u8], alg: HashAlg)
    -> [u8;32]} that computes the leaf hash (with the \texttt{0x00} prefix)
    over raw chunk bytes. \textbf{BLAKE3 API note:} use
    \texttt{Hasher::new().update(\&[0x00]).update(data)\\
    .finalize()} (the flat hash API); do \emph{not} use BLAKE3's internal
    tree mode (\texttt{new\_derive\_key} or implicit 1\,KB chunking), which
    does not expose intermediate nodes and cannot produce per-chunk proofs.
    BLAKE3 tree mode is benchmarked for throughput in RQ4 only.
  \item Write unit tests covering at minimum:
    \begin{itemize}[nosep]
      \item a single-leaf tree (the root must equal the leaf hash),
      \item a two-leaf tree (verify the root by hand),
      \item an eight-leaf tree whose expected root is pre-computed externally
        using a reference implementation.
    \end{itemize}
  \item Add a \texttt{cargo bench} microbenchmark comparing SHA-256, BLAKE3,
    and K12 at chunk sizes 64\,KB, 256\,KB, and 1\,MB. Save results to
    \texttt{benches/results/hash-bench-\$(hostname).txt} with CPU model and
    RAM size in the header.
  \item \emph{Done when:} All unit tests pass; benchmark output is committed.
\end{enumerate}

%% ── P1.3 ──────────────────────────────────────────────────────────────────
\paragraph{P1.3\enspace Hybrid storage layout (Weeks~4--6).}
\textit{Dependency:} P1.3 can begin as soon as P1.2 steps~1--3 (the
\texttt{MerkleTree} struct and domain-separation logic) are complete, which
should happen by the end of Week~3. P1.2's benchmark runs (step~5) may proceed
in parallel with P1.3.

Persist the Merkle tree alongside its dataset in the HDF5 file using the
two-tier layout defined in \S\ref{sec:merkle-storage}: a small root-hash
HDF5 attribute for fast checks, plus a companion dataset for the full node
array when $N > 256$ chunks.

\begin{enumerate}[nosep]
  \item In \texttt{clawhdf5-format}, implement
    \texttt{write\_merkle\_attr(dataset: \&Dataset, tree: \&MerkleTree)}.
    Pack the root hash (32 bytes), algorithm identifier (1 byte), and
    companion-integrity hash (32 bytes, see step~3) into a fixed-width binary
    blob and write it as the HDF5 attribute \texttt{\_merkle\_root} on the
    dataset.
  \item Implement \texttt{write\_merkle\_companion(file: \&File, name: \&str,
    tree: \&MerkleTree)}. For datasets with more than 256 chunks, write the
    complete \texttt{nodes} array as a flat \texttt{u8} dataset at path
    \texttt{/\_merkle/\{name\}}. For datasets with 256 or fewer chunks,
    pack all node hashes into the attribute directly (no companion dataset).
  \item Compute the companion-integrity hash: after writing the companion
    dataset, compute SHA-256 over its full byte content and store the result
    in the \texttt{\_merkle\_root} attribute. This single extra hash catches
    any direct tampering with the companion dataset before the tree is walked.
  \item Write a round-trip test: create a 1,024-chunk synthetic dataset, call
    the write functions, close the file, re-open it, read back the attribute
    and companion, and verify the companion-integrity hash matches the
    recomputed value.
  \item Confirm the companion dataset is visible using
    \texttt{h5dump -n} on the test file and that no existing ClawHDF5 tests
    regress (\texttt{cargo test --workspace}).
  \item \emph{Done when:} Round-trip test passes; \texttt{h5dump} shows the
    expected paths; no regressions.
\end{enumerate}

%% ── P1.3b ─────────────────────────────────────────────────────────────────
\paragraph{P1.3b\enspace Parallel leaf-hashing and Map-Reduce aggregation
(Weeks~5--7).}
\textit{Dependency:} requires P1.2 (tree struct) and P1.3 (storage layout).
The write-amplification analysis (\S\ref{sec:merkle-storage}) and R11
(distributed parallel construction) both require that leaf hashing is
embarrassingly parallel. Implement the Map-Reduce aggregator described
in~\S\ref{sec:merkle-storage}.

\begin{enumerate}[nosep]
  \item Add Cargo dependency \texttt{rayon} to \texttt{clawhdf5-format}.
  \item Implement \texttt{MerkleTree::build\_parallel(chunks: \&[\&[u8]],
    alg: HashAlg) -> Self}:
\begin{lstlisting}
// Map phase: hash all leaves in parallel
let leaf_hashes: Vec<[u8; 32]> = chunks
    .par_iter()
    .map(|c| hash_chunk(c, alg))
    .collect();
// Reduce phase: single-threaded tree aggregation
MerkleTree::build(&leaf_hashes, alg)
\end{lstlisting}
    The reduce phase is sequential because each internal node depends on
    its two children; the contention is at the upper tree levels, not the
    leaves.
  \item For the MPI-IO scenario (C~HDF5 Phase~3), document the equivalent
    distributed protocol: each MPI rank computes leaf hashes for its own
    chunk subset; ranks perform an \texttt{MPI\_Reduce} tree over the
    leaf-hash arrays using $\lceil\log_2 P\rceil$ rounds; the root rank
    aggregates the final tree. No global lock is needed because each rank
    owns a disjoint leaf range.
  \item Write a correctness test: verify that
    \texttt{build\_parallel} and \texttt{build} produce identical roots
    for a 1,024-chunk synthetic dataset on at least 4 rayon threads.
  \item Benchmark \texttt{build\_parallel} vs.\ \texttt{build} at
    $N \in \{10^4, 10^5, 10^6\}$ on the workstation. Save to
    \texttt{benches/results/parallel-build-\$(hostname).csv}.
  \item \emph{Done when:} Correctness test passes; benchmark is committed.
\end{enumerate}

%% ── P1.4 ──────────────────────────────────────────────────────────────────
\paragraph{P1.4\enspace Verification API (Weeks~7--10).}
Expose the \texttt{MerkleError} type and five public verification functions
in \texttt{clawhdf5-format/src/merkle.rs}. Define the error enum first:
\begin{lstlisting}
pub enum MerkleError {
    HashMismatch { chunk_idx: usize },  // leaf hash != recomputed value
    CompanionTampered,                  // companion-integrity hash mismatch
    SignatureInvalid,                   // hybrid sig verification failed
    MissingChunkGridMetadata,           // _merkle_root attribute absent
    HyperslabOutOfBounds { idx: usize },// chunk index exceeds dataset
    TreeTooDeep { depth: usize },       // depth > 40, reject to prevent OOM
    NoncePending,                       // WAL has uncommitted entry for chunk
}
\end{lstlisting}
Then expose the five public verification functions:
\begin{lstlisting}
pub fn verify_root(d: &Dataset) -> Result<bool, MerkleError>;
pub fn verify_chunk(d: &Dataset, idx: usize) -> Result<bool, MerkleError>;
pub fn verify_dataset(d: &Dataset) -> Result<bool, MerkleError>;
pub fn extend_merkle(d: &Dataset, idx: usize, h: [u8;32]) -> Result<(), MerkleError>;
pub fn update_merkle(d: &Dataset, idx: usize, h: [u8;32]) -> Result<(), MerkleError>;
\end{lstlisting}

\begin{enumerate}[nosep]
  \item \texttt{verify\_root}: reads the \texttt{\_merkle\_root} attribute,
    re-hashes the companion dataset bytes, and compares against the stored
    companion-integrity hash. Sub-second; does \emph{not} re-hash chunk
    data---only detects tampering with the companion dataset.
  \item \texttt{verify\_chunk}: re-hashes the live chunk at index \texttt{idx},
    reads the $O(\log N)$ sibling hashes from the companion dataset, and walks
    the proof path to the root, comparing the recomputed root to the stored value.
    If the WAL contains an uncommitted entry for \texttt{idx}, returns
    \texttt{Err(MerkleError::NoncePending)} immediately (the chunk may be
    partially written; the caller must replay or abort before verifying).
  \item \texttt{verify\_dataset}: re-hashes every chunk, rebuilds the tree
    from scratch, and compares the computed root to the stored root.
    This is the full $O(N)$ integrity check.
  \item \texttt{extend\_merkle}: appends a new leaf hash \texttt{h} at position
    \texttt{idx} and recomputes only the $O(\log N)$ nodes on the path to the
    root; leaves all other nodes unchanged.
  \item \texttt{update\_merkle}: same path recomputation as
    \texttt{extend\_merkle} but for an in-place overwrite of an existing chunk.
  \item Write tests using a 1,024-chunk synthetic dataset. Tamper one byte
    in one chunk on disk (using raw file I/O, bypassing ClawHDF5). Confirm:
    \texttt{verify\_chunk} returns \texttt{Err(MerkleError::HashMismatch)}
    for the tampered chunk; \texttt{verify\_dataset} also returns an error;
    \texttt{verify\_root} returns \texttt{Ok(true)} (correct---it only
    checks the companion, not chunk data).
  \item \emph{Done when:} All five functions compile and pass tests; the
    tamper-detection test produces exactly the expected error values.
\end{enumerate}

%% ── P1.5 ──────────────────────────────────────────────────────────────────
\paragraph{P1.5$\star$\enspace Verifiable subset extraction
(Weeks~8--13, \textbf{Move~1}).}
This is the first primary research contribution. A recipient of a hyperslab
must be able to prove cryptographically that the delivered chunks are a
\emph{complete and correct} subset of the signed parent dataset---not just
that each individual chunk is intact. Read \S\ref{sec:subset} thoroughly
before starting; the coverage certificate is what distinguishes this from a
plain Merkle inclusion proof.

\begin{enumerate}[nosep]
  \item In \texttt{clawhdf5-format/src/subset\_proof.rs}, define:
\begin{lstlisting}
pub struct ChunkGridParams {
    pub dims:        Vec<u64>,   // dataset dimensions
    pub chunk_shape: Vec<u64>,   // chunk dimensions
    pub grid_hash:   [u8; 32],   // H(dims || chunk_shape)
}

pub struct SubsetProof {
    pub chunk_indices:      Vec<usize>,
    pub leaf_hashes:        Vec<[u8; 32]>,
    // Deduplicated proof nodes keyed by 1D level-order tree index.
    // Using BTreeMap (not Vec<Vec<...>>) because after deduplication
    // adjacent proof paths share internal nodes; a flat nested Vec cannot
    // express which node sits at which tree position, breaking path
    // reconstruction at the verifier.  Key = level-order index in the
    // padded binary tree (root = 0, left child of i = 2i+1, right = 2i+2).
    pub proof_nodes:        std::collections::BTreeMap<u64, [u8; 32]>,
    pub grid_params:        ChunkGridParams,
    pub coverage_cert:      [u8; 32],  // H(sorted_indices || grid_hash)
}
\end{lstlisting}
  \item Implement \texttt{extract\_subset(dataset: \&Dataset,}
    \texttt{hyperslab: \&Hyperslab)} returning
    \texttt{Result<SubsetProof, MerkleError>}. Translate
    the hyperslab into a sorted list of chunk indices; compute each leaf's
    proof path as a set of level-order tree indices; take the union of all
    path index sets and populate \texttt{proof\_nodes} with one entry per
    unique index---this is the deduplication step that keeps $|\pi|$
    compact for contiguous hyperslabs. Build the coverage certificate by
    hashing the sorted index list together with the grid-parameter hash.
    The verifier reconstructs each path by looking up the required nodes
    from \texttt{proof\_nodes} using level-order index arithmetic, so every
    node that appears in more than one proof path is transmitted only once.
  \item Implement \texttt{verify\_subset(root: [u8;32], chunks: \&[ChunkData],
    proof: \&SubsetProof) -> Result<bool, MerkleError>}. On the recipient
    side: re-hash each delivered chunk, walk its sibling path to confirm it
    is included under \texttt{root}, and verify the coverage certificate
    confirms no requested chunk is missing or substituted.
  \item Measure proof size~$|\pi|$ (bytes) for three hyperslab shapes on a
    dataset with $N = 65{,}536$ chunks:
    \begin{itemize}[nosep]
      \item a contiguous rectangular slab (best case, many shared nodes),
      \item a strided slab (fewer shared nodes),
      \item a random sparse selection of $k$ chunks (near-worst case).
    \end{itemize}
    Record results and compare against the theoretical bound $O(k \log N)$
    (RQ6). Save the table to \texttt{benches/results/subset-proof-size.csv}.
  \item \emph{Done when:} \texttt{verify\_subset} returns \texttt{Ok(true)}
    for a correct proof; returns \texttt{Err} when one delivered chunk is
    modified; returns \texttt{Err} when one requested chunk is silently
    omitted from the delivered set. All three failure modes must be covered
    by explicit tests.
\end{enumerate}

%% ── P1.6 ──────────────────────────────────────────────────────────────────
\paragraph{P1.6\enspace Phase~1 benchmarks (Weeks~13--16).}
Compare Merkle-tree overhead against ClawHDF5's existing flat SHA-256
verification to answer RQ1, RQ2, and RQ3.

\begin{enumerate}[nosep]
  \item Using the 10\,GB synthetic file from P1.1, run and record wall-clock
    time and peak RSS for each of the following scenarios:
    \begin{itemize}[nosep]
      \item \texttt{verify\_dataset} (full Merkle re-hash) vs.\ existing
        flat \texttt{verify\_dataset()} on the same file,
      \item \texttt{verify\_chunk} for a single chunk (demonstrates partial
        verification; flat hashing cannot do this),
      \item companion dataset I/O cost vs.\ attribute-only storage at chunk
        counts $N \in \{16,\,256,\,1{,}024,\,65{,}536\}$.
    \end{itemize}
  \item Repeat on a real NOAA dataset.
  \item Save all raw numbers, the hostname, CPU model, and disk type to
    \texttt{benches/results/phase1-\$(hostname).csv} and commit the file.
  \item \emph{Done when:} CSV files are committed; numbers are ready to
    paste into the paper's evaluation tables (\S\ref{sec:experiments}).
\end{enumerate}

%% ── Phase 2 ──────────────────────────────────────────────────────────────────
\subsection{Phase 2: Security Contributions and Evaluation (Months 4--7)}

\noindent\textit{Background reading before starting Phase~2:} read
\S\ref{sec:signing} (Signing Authority and Key Management), paying particular
attention to the ``Post-quantum hybrid signing'' paragraph (\S\ref{sec:pq-hybrid})
for the hybrid Ed25519~$+$~ML-DSA-65 construction and payload encoding; then
\S\ref{sec:filter-order} (Encrypt-then-MAC filter pipeline ordering and why
hash-then-encrypt is insecure), \S\ref{sec:nonce} (version-counter nonce
derivation and why it prevents nonce reuse on in-place updates), and
\S\ref{sec:security-analysis} (the game-based argument template, including the
three component security assumptions).

%% ── P2.1 ──────────────────────────────────────────────────────────────────
\paragraph{P2.1$\star$\enspace Hybrid Ed25519~$+$~ML-DSA-65 signing
(Weeks~1--4 of Phase~2, \textbf{Move~2}).}
This is the second primary research contribution: a post-quantum-safe signing
scheme that attaches a non-repudiable author identity to each Merkle root and
survives the 50-year archival timescales typical of scientific data.

\begin{enumerate}[nosep]
  \item Create a \texttt{clawhdf5-sign} crate (or module inside
    \texttt{clawhdf5-format}). Add Cargo dependencies:
    \texttt{ed25519-dalek} and either \texttt{ml-dsa} or
    \texttt{pqcrypto-mldsa}.
  \item Implement the canonical payload encoder
    (see \S\ref{sec:pq-hybrid} for field definitions):
\begin{lstlisting}
fn canonical_payload(
    root:           &[u8; 32],
    companion_hash: &[u8; 32],
    version:        u64,       // dataset version counter
    timestamp:      u64,       // Unix seconds
    alg_id:         HashAlg,
) -> Vec<u8>
// Returns: big-endian fixed-width binary encoding, no separators.
\end{lstlisting}
  \item Implement \texttt{sign\_root(payload: \&[u8], ed\_key:
    \&Ed25519SigningKey, ml\_key: \&MlDsaPrivateKey) -> HybridSignature}.
    Produce both signatures independently over the same payload and store
    them concatenated with a 1-byte version tag in the HDF5 attribute
    \texttt{\_merkle\_sig} on the dataset.
  \item Implement \texttt{verify\_sig(payload: \&[u8], sig:
    \&HybridSignature, ed\_pub: \&Ed25519VerifyingKey, ml\_pub:
    \&MlDsaPublicKey) -> Result<(), SigError>}. \textbf{Require both
    signatures to be valid}; accept the payload only if neither fails.
  \item Measure sign and verify latency on x86-64 (with and without
    AVX-512) and on ARM. Vary dataset size from $10^4$ to $10^7$ chunks.
    Record signature size, public-key size, and object-header size impact
    (RQ7). Save to \texttt{benches/results/signing-\$(hostname).csv}.
  \item \emph{Done when:} Round-trip sign/verify test passes; a test with
    one byte of the payload modified returns
    \texttt{Err(SigError::InvalidSignature)}; benchmarks are committed.
\end{enumerate}

%% ── P2.2 ──────────────────────────────────────────────────────────────────
\paragraph{P2.2\enspace ChaCha20-Poly1305 AEAD filter and nonce derivation
(Weeks~2--5 of Phase~2).}
Add the encryption layer to \texttt{clawhdf5-filters} and implement the
version-counter nonce derivation that makes in-place chunk updates safe.
Read \S\ref{sec:filter-order} and \S\ref{sec:nonce} before starting.

\begin{enumerate}[nosep]
  \item \textbf{Key management scope for P2.2.}
    ClawHDF5 does not yet have key management infrastructure; Task~2 of
    S2-D2 will provide the production KeyStore. For this prototype phase,
    implement a \texttt{MockKeyStore} that derives a static 32-byte DEK
    from the environment variable \texttt{CLAW\_DUMMY\_DEK} (hex-encoded):
\begin{lstlisting}
fn mock_dek() -> [u8; 32] {
    let hex = std::env::var("CLAW_DUMMY_DEK")
        .expect("set CLAW_DUMMY_DEK for prototype");
    hex::decode(&hex).unwrap().try_into().unwrap()
}
\end{lstlisting}
    Do \emph{not} attempt to build HKDF/KEK wrapping or multi-party key
    distribution in P2.2; that is deferred to Phase~3. All tests must set
    \texttt{CLAW\_DUMMY\_DEK} in the test environment (add it to
    \texttt{.cargo/config.toml} under \texttt{[env]}).
  \item Add a \texttt{chacha20\_filter} module to \texttt{clawhdf5-filters}
    with Cargo dependency \texttt{chacha20poly1305}.
  \item Implement version-counter nonce derivation. The nonce for encrypting
    chunk~$i$ at write number~$v$ is:
    $\mathit{nonce} = \mathrm{BLAKE3\text{-}derive}(\mathrm{DEK},\;
    \mathit{chunk\_idx} \,\|\, v_\mathrm{chunk})$.
    The per-chunk write counter $v_\mathrm{chunk}$ is stored as a
    \texttt{u64} in the companion dataset alongside each leaf hash.
    \textbf{Use the WAL protocol from \S\ref{sec:nonce}}: journal
    $(\mathit{chunk\_idx}, v_\text{new})$ to a crash-durable log
    \emph{before} deriving the nonce; write the chunk using $v_\text{new}$;
    then update the companion dataset and mark the journal record committed.
    Never increment $v_\mathrm{chunk}$ directly in the companion dataset
    before writing the chunk, and never reuse the old $v_\mathrm{chunk}$
    after a crash. A nonce must never be reused under the same DEK.
  \item Enforce the Encrypt-then-MAC pipeline order for filters:
    compress $\to$ shuffle $\to$ encrypt. The Merkle leaf hash must be
    computed over the ciphertext output of the filter chain, not over the
    plaintext (\S\ref{sec:filter-order}).
  \item Bind the AEAD authentication tag and the version counter into the
    Merkle leaf:
    $\mathrm{leaf}[i] = H\!\left(\texttt{0x00} \,\|\,
    \mathrm{ciphertext} \,\|\, \mathrm{tag} \,\|\, v_\mathrm{chunk}\right)$.
  \item Write an end-to-end encrypted test: write a 256-chunk encrypted
    dataset, verify the Merkle tree, overwrite chunk~7 (which must increment
    $v_\mathrm{chunk}[7]$ and update the leaf hash), then call
    \texttt{verify\_dataset} and confirm the tree is still consistent.
  \item \emph{Done when:} The end-to-end test passes. Add a short code
    comment proving nonce reuse is impossible by construction (``nonce
    reuse requires $v_\mathrm{chunk}$ rollback, which is prevented by
    $\ldots$'').
\end{enumerate}

%% ── P2.3 ──────────────────────────────────────────────────────────────────
\paragraph{P2.3\enspace Game-based security argument
(Weeks~4--7 of Phase~2, \textbf{Move~4}).}
This is a written deliverable, not a code task. Its goal is to give a
rigorous argument that breaking the full scheme requires breaking one of
its three components (AEAD, Merkle tree, or hybrid signature). Read
\S\ref{sec:security-analysis} for the template.

\begin{enumerate}[nosep]
  \item Write the INT-CTXT (integrity under chosen-ciphertext attack) security
    game for the combined EtM~$+$~version-counter~$+$~hybrid-signed-root
    construction in the style of Bellare and Namprempre.
  \item State the three component security assumptions: IND-CCA2 / INT-CTXT
    for ChaCha20-Poly1305, collision resistance for the Merkle hash, and
    CMA (chosen-message attack) security for Ed25519 and ML-DSA-65.
  \item Give the reduction argument: an adversary who forges an
    integrity proof for a modified file must either (a)~find a hash
    collision, (b)~decrypt and re-encrypt a chunk without the DEK, or
    (c)~forge a signature without the private key.
  \item Write the argument as a self-contained appendix section in the
    paper draft and request a senior-researcher review before finalizing.
  \item \emph{Done when:} The section is reviewed and the paper's
    \S\ref{sec:security-analysis} is complete.
\end{enumerate}

%% ── P2.4 ──────────────────────────────────────────────────────────────────
\paragraph{P2.4$\star$\enspace Adversarial red-team study
(Weeks~5--9 of Phase~2, \textbf{Move~3}).}
\textit{Dependency:} P2.1 (signing) must be complete before starting
(done by end of Week~4). P2.2 (AEAD filter) finishes at end of Week~5;
attacks involving the encryption layer (T1 compressed-payload attacks, T4
nonce-reuse attempts) should be deferred to Week~6 if P2.2 is not yet
merged, while plaintext-dataset attacks (T2, T3, T5--T8) can begin at
Week~5.

This is the third primary research contribution. Attempt to break the
implementation on real scientific datasets using the eight threat classes
defined in \S\ref{sec:threat} (T1--T8).

\begin{enumerate}[nosep]
  \item Add an \texttt{attack-harness} binary to the repository. Each attack
    opens a test HDF5 file, performs one targeted modification using raw
    file I/O (bypassing ClawHDF5), then calls the verification API and
    asserts the expected result.
  \item Implement and run each attack class on the NOAA dataset:
    \begin{itemize}[nosep]
      \item \textbf{T1} (storage tampering): overwrite raw chunk bytes using
        a hex editor; \texttt{verify\_chunk} must detect it.
      \item \textbf{T2} (silent corruption): flip one bit in the companion
        dataset; \texttt{verify\_root} must detect it via the
        companion-integrity hash.
      \item \textbf{T3} (root substitution): replace \texttt{\_merkle\_root}
        with a freshly computed root for the tampered file;
        \texttt{verify\_sig} must reject it (no valid signature on the
        new root).
      \item \textbf{T4} (replay): copy the \texttt{\_merkle\_sig} from an
        older file version onto the current file; the version counter in
        the canonical payload must cause rejection.
      \item \textbf{T5--T8}: metadata tampering, missing-chunk substitution,
        and subset-extraction attacks as defined in \S\ref{sec:adversarial}.
    \end{itemize}
  \item Repeat the full harness on the EQSIM dataset.
  \item Record the detected/undetected result for each attack on each
    dataset in \texttt{attack-results/matrix.csv}. Any \emph{undetected}
    result requires either a code fix or a documented limitation explaining
    why the attack is out of scope.
  \item \emph{Done when:} All T1--T8 attacks are detected on both datasets;
    the matrix CSV is committed with zero unresolved \emph{undetected} entries.
\end{enumerate}

%% ── P2.5 ──────────────────────────────────────────────────────────────────
\paragraph{P2.5\enspace Cloud / WAN evaluation
(Weeks~8--11 of Phase~2, \textbf{Move~6}).}
Measure proof-path retrieval latency over S3 range-GETs. Read
\S\ref{sec:cloud-eval} for the full measurement protocol.

\begin{enumerate}[nosep]
  \item Wire the companion-dataset proof retrieval into the S3 range-GET
    path already present in \texttt{clawhdf5-io}. A single proof path for
    one chunk requires only $O(\log N)$ 32-byte node reads; issue them as
    one range request covering the relevant slice of the companion dataset.
  \item Upload representative test files (1\,GB and a larger surrogate for
    1\,TB-scale behavior) to the S3 test bucket.
  \item Run the intra-region and cross-region WAN measurements from
    \S\ref{sec:cloud-eval}: record first-chunk proof latency, full-dataset
    verify latency, and total bytes transferred per verification call.
  \item Save results to \texttt{benches/results/phase2-cloud.csv} and commit.
  \item \emph{Done when:} CSV is committed; numbers are ready for the
    paper's cloud-evaluation table.
\end{enumerate}

%% ── P2.6 ──────────────────────────────────────────────────────────────────
\paragraph{P2.6\enspace C~HDF5 integration design document
(Weeks~10--12 of Phase~2).}
A technical specification describing how the Merkle-tree storage format
and verification API would be added to the official C~HDF5 library.

\begin{enumerate}[nosep]
  \item Describe the on-disk layout: root-hash attribute
    (\texttt{\_merkle\_root}) plus companion dataset
    (\texttt{/\_merkle/\{name\}}), with attribute-only fallback for
    $N \leq 256$ chunks (\S\ref{sec:merkle-storage}).
  \item Specify proposed new public API functions:
    \texttt{H5Dverify\_chunk},
    \texttt{H5Dextract\_\allowbreak{}subset}, and
    \texttt{H5Fget\_\allowbreak{}integrity\_root},
    with C function signatures and error-code semantics.
  \item Specify companion-dataset preservation semantics for
    \texttt{h5copy}, \texttt{h5repack}, and hyperslab-copy operations
    (the companion must be updated atomically with the data).
  \item Specify the crash-consistency write order:
    chunks $\to$ companion dataset $\to$ root attribute. Writes in the
    reverse order must never be generated; document how each API call
    enforces this.
  \item Specify filter-pipeline ordering requirements from
    \S\ref{sec:filter-order} and the on-disk layout of the hybrid
    signed-root attribute.
  \item Get an internal review from a senior team member before submitting.
\end{enumerate}

%% ── P2.7 ──────────────────────────────────────────────────────────────────
\paragraph{P2.7\enspace HDF5 community RFC (Weeks~11--13 of Phase~2).}

\begin{enumerate}[nosep]
  \item Summarize the on-disk format and proposed API from P2.6 in a
    self-contained document without implementation details or proofs.
    Target length: 4--6 pages.
  \item Post to the HDF5 forum and record all substantive feedback in a
    \texttt{rfc/feedback.md} file in the repository.
  \item \emph{Done when:} RFC is posted and the feedback file is committed.
\end{enumerate}

%% ── Phase 3 ──────────────────────────────────────────────────────────────────
\subsection{Phase 3 / Stretch}

These items are cut-able without affecting the three primary paper
contributions. Begin them only after Phase~2 is complete and on schedule.
Items are in priority order.

\begin{enumerate}[nosep]
  \item \textbf{Append-only log Merkle mode (Move~5):} implement the
    Certificate-Transparency-style append-only variant: each new dataset
    version appends leaves and
    stores a consistency proof that version~$n$ is a true prefix of
    version~$m$. Targets time-series and streaming science workloads.
  \item \textbf{Tamarin symbolic model:} encode the verification
    protocol in Tamarin~Prover and prove authenticity, freshness, and
    subset-soundness lemmas mechanically. This elevates the P2.3
    game-based argument to a machine-checked proof suitable for a
    security-venue submission.
  \item \textbf{Sigstore / Rekor transparency-log integration:} publish
    hybrid signed roots to a Rekor log and store RFC~3161 TSA tokens for
    Long-Term Validation.
  \item \textbf{AES-256-CTR~$+$~HMAC-SHA-256 filters:} FIPS-compliant
    alternative to ChaCha20-Poly1305 for deployments subject to FIPS~140-3.
...
\textbf{Target venue.} PDSW @ SC'26 (10~pp) is the primary target at current
scope; submission deadline is approximately August~2026. CCGrid / IPDPS
(12~pp) is the backup if scope expands: both conferences are held in
\emph{May}, so submission deadlines fall in the preceding fall/winter
(IPDPS~2027: $\sim$October~2026; CCGrid~2027: $\sim$November--December~2026).
USENIX Security / ACSAC if the Tamarin model is completed.
...
Then: requirements R1--R12 derived from HDF5 access patterns and archival
...
\centering
\small
\begin{tabular}{@{}p{1.1cm}p{6.8cm}cccc@{}}
\toprule
...
\textbf{$\sim$August 2026} & \textbf{Submit} &
  \textbf{PDSW @ SC'26 submission deadline (primary target).}
  If rejected or scope expands: CCGrid\,/\,IPDPS~2027 deadlines fall
  Oct--Dec~2026 (conferences are in May). \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{CCGrid / IPDPS (12\,pp) --- backup if scope expands:}
    Full 8-RQ version with comfortable page budget. Both conferences are
    held in May; submission deadlines for the 2027 editions fall in the
    preceding fall/winter: IPDPS~2027 $\sim$October~2026, CCGrid~2027
    $\sim$November--December~2026. These deadlines are compatible with
    the August~2026 PDSW submission: if PDSW is rejected, a revised
    version can be submitted to CCGrid/IPDPS on the same cycle.
