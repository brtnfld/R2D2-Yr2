...
\lhead{\small Tamper Detection and Data Integrity for Scientific Data}
\cfoot{\thepage}
...
satisfies all of these requirements at petabyte scale, and we then design,
implement, and evaluate such a scheme in RustyHDF5~\cite{rustyhdf5}. Beyond
...
directed-adversary red-team study on real scientific datasets. The RustyHDF5
...
Scientific HDF5 datasets routinely reach terabyte to petabyte scale, are
retained for decades, and are consumed by readers who access only small
hyperslabs at a time. They live on shared parallel filesystems, in cloud
...
an adversary. RustyHDF5 includes a basic provenance feature that computes a
single SHA-256 hash per dataset, but this requires rehashing the entire
dataset to verify integrity---infeasible for petabyte-scale data where a
researcher may access only a small hyperslab. Neither the C~HDF5 library nor
RustyHDF5 currently provides a mechanism for \emph{partial verification}
...
requirements at petabyte scale, and we devote the remainder of the document
(\S\ref{sec:merkle} onward) to designing, implementing, and evaluating such
a construction.

\paragraph{Experimental platform.}
This work uses RustyHDF5~\cite{rustyhdf5}---a pure-Rust HDF5 implementation
...
each subsection with the suitability of the approach for petabyte-scale,
...
iSCSI~\cite{rfc3385} and Btrfs, Adler-32, Fletcher-32), the Jenkins lookup3
hash used by HDF5 v2 metadata pages, and Reed-Solomon codes~\cite{reedsolomon}
detect random bit errors arising from hardware faults, transmission errors,
or filesystem bugs~\cite{koopman2002crc}. They are extremely fast (often
hardware-accelerated), compact (4--8\,B), and widely deployed in storage and
networking. The HDF5 fletcher32 chunk filter is in this family. However, all
listed codes are linear over $\mathrm{GF}(2)$ and can be inverted in closed
form by an adversary: given a desired alteration, the attacker computes a
matching tail correction that preserves the checksum. They therefore detect
...
sub-dataset partial verification is possible from a flat hash. RustyHDF5's
existing flat-SHA-256 provenance feature is in this category.

\subsection{Symmetric-key authentication: MACs and AEAD}
...
shared symmetric key. Tags are compact (16\,B). HMAC-SHA-256 and Poly1305 run
at multi-GB/s per core. Per-chunk MAC or AEAD provides chunk-level adversarial
integrity at minimal cost. The fundamental limitation is that verification
requires possession of the secret key: a third-party auditor, a public archive
mirror, or a downstream researcher receiving a derived hyperslab cannot verify
without it. Symmetric authentication is therefore appropriate as a
\emph{component} (e.g., AEAD ciphertext authenticity) but cannot be the public
provenance primitive on its own. Encrypt-then-MAC composition is the canonical
secure pattern~\cite{bellare2000encrypt,bellare2000encryptmac}.
...
$N=10^9$ chunks, per-chunk signatures consume $64$--$512$\,GB and require
commensurate signing time. BLS aggregate signatures reduce storage at the
...
forge-later} resistant. NIST has standardized two post-quantum
signature schemes for general use: ML-DSA (Module-Lattice DSA, formerly
Dilithium)~\cite{fips204} and SLH-DSA (Stateless Hash-based DSA, formerly
SPHINCS$^+$)~\cite{fips205}. Stateful hash-based schemes XMSS~\cite{rfc8391}
...
commit; none are competitive with hash trees for our setting at petabyte
scale.

\subsection{Zero-knowledge proofs of integrity}

ZK-SNARKs (Groth16, PLONK, Marlin) and ZK-STARKs prove correctness of
arbitrary computation, including correctness of an integrity claim, with
minimal proof size. Proofs of Data Possession~\cite{ateniese2007pdp} and
Proofs of Retrievability~\cite{juels2007por} let a verifier audit remote
storage without retrieving full data, by sampling random blocks and checking
authenticator tags. These primitives are most useful when (a)~one wishes to
hide intermediate values from the verifier, or (b)~one wishes to perform
\emph{spot-checks} rather than complete verification. Both situations are
distinct from our problem (researchers want full guarantees over the data
they actually read) and impose substantial proving cost. They remain candidates
for orthogonal extensions (e.g., a sampling-based audit of an archive's
overall integrity).
...
integrity tied to platform-binding keys. dm-verity~\cite{dmverity} (Android
read-only partitions) and fs-verity~\cite{fsverity} (Linux per-file Merkle
trees enforced by the kernel) bind such measurements to filesystem objects.
TEE-anchored memory integrity trees are also used inside the enclave itself.
These mechanisms are powerful within a controlled platform but rely on
recurrent vulnerability disclosures, vendor-specific attestation services,
and the integrity of a particular hardware instance---assumptions
incompatible with multi-decade archival of scientific data that may be
read on platforms not yet built.

\subsection{Filesystem and block-level integrity}

ZFS~\cite{bonwick2003zfs} stores Fletcher4 or SHA-256 checksums for every
block and arranges them in a Merkle-style hierarchy that enables silent-corruption
detection and self-healing from redundant copies. Btrfs uses CRC-32C per
block. dm-integrity (LUKS) provides per-block AEAD. dm-verity, fs-verity,
and Linux IMA bind kernel-enforced Merkle hashes to read-time policy. These
are excellent within their respective trust boundaries, but the integrity
guarantee evaporates the moment data leaves the filesystem (e.g., copied to
S3, archived to tape, transmitted across a WAN). For a portable, archive-grade
guarantee, the integrity must travel \emph{with} the file rather than reside
in the storage substrate.
...
commitments (trusted setup or post-quantum-broken or both), and
filesystem/cloud-store mechanisms (not portable). \S\ref{sec:selection}
...
\begin{description}[nosep]
  \item[R1: Sub-dataset partial verification.]
    ...
    without reading or hashing the remaining $N-k$ chunks. (Petabyte-scale
    rehashing is infeasible.)
  ...
  \item[R5: Verifiable subset extraction.]
    A recipient of a hyperslab plus a small auxiliary proof must be able to
    cryptographically verify that the hyperslab is a true subset of the
    signed parent dataset, without access to the rest of the dataset.
  ...
  \item[R10: HPC-feasible build cost.]
    Initial commitment over a petabyte-scale dataset must be feasible at
...
Table~\ref{tab:integrity-comparison} maps the seven candidate classes
identified in the survey against these ten requirements.
...
\midrule
R1 partial verification & $\times$   & \checkmark & \checkmark & \checkmark & \checkmark & sample-only & \checkmark & \checkmark \\
R2 incremental extend   & \checkmark & \checkmark & \checkmark & costly     & costly     & costly      & \checkmark & \checkmark \\
R3 in-place update      & $\times$   & \checkmark & \checkmark & costly     & costly     & costly      & \checkmark & \checkmark \\
R4 tamper localization  & $\times$   & \checkmark & \checkmark & $\times$   & partial    & $\times$    & \checkmark & \checkmark \\
R5 subset extraction    & $\times$   & $\times$   & $\times$   & complex    & \checkmark & \checkmark  & $\times$   & \checkmark \\
R6 public verifiability & \checkmark & $\times$   & \checkmark & \checkmark & \checkmark & \checkmark  & vendor     & \checkmark \\
R7 compact subset proof & $\times$   & $\times$   & $\times$   & $O(1)$     & $O(1)$     & $O(1)$      & ---        & $O(\log N)$ \\
...
R10 HPC-feasible build  & \checkmark & \checkmark & costly     & $\times$   & $\times$   & $\times$    & \checkmark & \checkmark \\
\bottomrule
\end{tabular}
\caption{Candidate primitives against the requirements of
\S\ref{sec:requirements}. ``vendor'' = depends on platform vendor; ``costly''
= achievable but with non-trivial overhead; ``sample-only'' = provides
sampling-based assurance rather than complete verification.
$\dagger$~Per-chunk signatures upgrade to PQ by replacing the scheme but
remain storage-prohibitive.}
...
signed by a hybrid classical~+~PQ signature, is the only candidate that
satisfies R1--R10 simultaneously:

\begin{itemize}[nosep]
  \item Subset proofs of size $O(k\log N)$ (R5, R7) come for free from the
    tree structure; subset extraction (R5) reduces to a Merkle multiproof.
  \item Tamper localization (R4) walks from root to leaves in $O(\log N)$.
  \item In-place update (R3) and append (R2) recompute only the
    $O(\log N)$ hashes on the affected root path, not the whole tree.
  \item No trusted setup (R8); a hash tree is post-quantum simply by choosing
    a PQ-secure hash (R9) and binding the root with a PQ signature.
  \item Build cost (R10) is one hash per chunk plus $N-1$ pair hashes for
    internal nodes, executable at memory-bandwidth speed on existing HPC
    hardware (SHA-NI, AVX-512, NEON).
  \item Public verifiability (R6) is provided by the asymmetric root
    signature.
\end{itemize}

The competitor primitives all fail at least one core requirement. Per-chunk
MACs and AEAD lose R6 and R7. Per-chunk signatures lose R7 and R10. Flat
hashing loses R1, R3, R4, R5. RSA accumulators and bilinear/KZG schemes lose
R8 and R9 and have no natural ordered-coverage proof (weakening R5).
ZK-SNARKs, PoR, and PDP either trade completeness for sampling (weakening R1
and R4) or carry trusted-setup or pairing-based PQ liabilities (R8, R9).
TEE/fs-verity loses portability across the multi-decade archival horizon and
binds verification to specific vendors and platforms.
