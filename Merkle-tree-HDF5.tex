...
developed under Task~2's plugin-signature work, PR~\#6198).
...
However, other dataset representations are discussed in Section \S\ref{sec:limitations}.
...
\paragraph{Why not alternative integrity structures?}
Several alternatives to Merkle trees exist for dataset authentication, each
with a disqualifying trade-off in this setting.
\emph{Flat hash lists} (one hash per chunk, no aggregation) are the approach
taken by Zarr and ASDF. They enable O(1) per-chunk verification but cannot
produce a compact subset-coverage proof: to confirm that a delivered hyperslab
is \emph{complete}, the receiver needs all $N$ chunk hashes to recompute
which chunks were required---an O(N) communication cost that defeats the
purpose of partial verification at petabyte scale.
\emph{Cryptographic accumulators}~\cite{benaloh1993accumulator} (RSA or
bilinear-map variants) achieve O(1) witness size but require a trusted setup
(a safe RSA modulus or a structured reference string from an MPC ceremony),
carry no post-quantum security, and are set-based rather than ordered, making
hyperslab coverage certificates difficult to construct without additional
machinery.
\emph{KZG polynomial commitments}~\cite{kate2010kzg} and Verkle trees reduce
multiproof size to a single constant-size group element but inherit the same
trusted-setup requirement, rely on elliptic-curve pairings that are broken by
quantum adversaries, and require O(N) multi-scalar multiplications to build the
commitment---computationally infeasible for petabyte-scale scientific datasets.
\emph{Per-chunk asymmetric signatures} (Ed25519 or ML-DSA-65 on every chunk)
eliminate the need for a tree entirely but produce $64N$--$3300N$ bytes of
signatures ($64\,\text{GB}$--$3.3\,\text{TB}$ at $N=10^9$ chunks) and require
commensurate signing time.
\emph{Per-chunk symmetric MACs} are fast and compact but require a shared
secret key, providing no public verifiability for third-party auditors.
A hash-based Merkle tree avoids all of these deficiencies: it requires no
trusted setup, is post-quantum upgradeable by swapping the hash function,
builds at memory-bandwidth speed, produces O($\log N$) subset proofs, and
enables a coverage certificate with a single signed root hash.
Table~\ref{tab:integrity-comparison} summarizes the trade-offs.

\begin{table}[h]
\centering
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lcccc>{\columncolor{blue!12}}c}
\toprule
\textbf{Property}
  & \textbf{\shortstack{Flat\\hash list}}
  & \textbf{\shortstack{Crypto\\accumulator}}
  & \textbf{\shortstack{KZG /\\Verkle}}
  & \textbf{\shortstack{Per-chunk\\signature}}
  & \textbf{\shortstack{Merkle\\tree}} \\
\midrule
No trusted setup          & \checkmark & $\times$ & $\times$ & \checkmark & \checkmark \\
Post-quantum ready        & \checkmark & $\times$ & $\times$ & $\dagger$  & \checkmark \\
$O(\log N)$ subset proof  & $\times$   & $O(1)$   & $O(1)$   & $\times$   & \checkmark \\
Coverage certificate      & $\times$   & complex  & complex  & $\times$   & \checkmark \\
Builds at hash speed      & \checkmark & $\times$ & $\times$ & $\times$   & \checkmark \\
HPC tooling exists        & \checkmark & $\times$ & $\times$ & \checkmark & \checkmark \\
Single root to sign       & $\times$   & \checkmark & \checkmark & ---     & \checkmark \\
\bottomrule
\end{tabular}
\caption{Comparison of dataset authentication approaches.
  $\dagger$~Requires replacing classical scheme with ML-DSA-65.}
\label{tab:integrity-comparison}
\end{table}

\paragraph{Authenticated data structures.}
Tamassia~\cite{tamassia2003authenticated} formalized authenticated data
...
\par\noindent\rule{\linewidth}{0.4pt}
\mapsec{Plan: Phase~2 items P2.3--P2.4; Phase~3 item P3.2 (stretch)}{RQ8}{T1--T5}
...
\midrule
P1.1 & Platform eval go/no-go (RustyHDF5 gate)
                                                        & \checkmark & & & \\
...
P2.3 & Game-based INT-CTXT security argument
                                                        & & \checkmark & & \\
P2.4\,$\star$ & Adversarial red-team study (T1--T5 + subset attacks)
