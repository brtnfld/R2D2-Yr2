...
\end{lstlisting}
  \item Implement \texttt{MerkleTree::build(leaves: \&[[u8;32]], alg: HashAlg)
    -> Self}. Apply domain-separation prefixes exactly as specified in
...
%% ── P1.4 ──────────────────────────────────────────────────────────────────
\paragraph{P1.4\enspace Verification API (Weeks~6--9).}
Expose five public functions that cover the full range of verification
granularities described in \S\ref{sec:merkle}. Add these signatures to
\texttt{clawhdf5-format/src/merkle.rs}:
...
pub struct SubsetProof {
    ...
    pub sibling_paths: Vec<Vec<[u8; 32]>>,  // one path per chunk
...
\end{lstlisting}
  \item Implement \texttt{extract\_subset(dataset: \&Dataset,}
    ...
    the hyperslab into a sorted list of chunk indices, fetch the Merkle
    sibling paths for each, deduplicate internal nodes shared by adjacent
    proof paths (to keep $|\pi|$ compact), and build the coverage certificate
    by hashing the sorted index list together with the grid-parameter hash.
...
\begin{enumerate}[nosep]
  ...
  \item Implement version-counter nonce derivation. The nonce for encrypting
    ...
    \textbf{Increment and persist $v_\mathrm{chunk}$ before writing the
    encrypted chunk}; a nonce must never be reused under the same DEK.
