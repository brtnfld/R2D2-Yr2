...
R10 HPC-feasible build  & \checkmark & \checkmark & 3\,h$^h$   & 3\,h$^a$   & 1\,s$^a$   & 1\,h$^b$   & \checkmark & 30\,s \\
\bottomrule
\end{tabular}
...
single-CPU time at $N=10^7$ chunks (10\,TB at 1\,MB chunks); Merkle baseline
is $\sim$30\,s of SHA-256 with SHA-NI. ``vendor'' = depends on platform
...
$^b$~ZK folding step (e.g., Nova/HyperNova) takes tens of ms per step;
aggregating $10^7$ steps approaches $\sim$1\,h.
...
\begin{itemize}[nosep]
  ...
  \item In-place update (R3) and append (R2) recompute only the
    $O(\log N)$ hashes on the affected root path---$\sim$30 SHA-256
    invocations ($\sim$3\,$\mu$s on SHA-NI) per update at $N=10^7$.
  ...
  \item Build cost (R10) is one hash per chunk plus $N-1$ pair hashes for
    internal nodes---$\sim$30\,s of SHA-NI throughput at $N=10^7$, two
    orders of magnitude below the algebraic alternatives.
...
aggregate and would otherwise satisfy R1--R6, R10. BLS is disqualified by R8
(BLS12-381 uses a structured reference setup) and R9 (pairings are not
post-quantum). Aggregate verification still requires the verifier to combine
$N$ public-key/message pairs to recompute the aggregate, and R5b (coverage
against the parent dataset) is not provided without an additional layer that
again reduces to a hash tree.
