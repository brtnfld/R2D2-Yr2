...
\begin{tikzpicture}[
  ...
  hash/.style={rectangle, draw, rounded corners, minimum width=1.8cm,
               minimum height=0.6cm, font=\scriptsize\ttfamily},
  chunk/.style={rectangle, draw, fill=blue!10, minimum width=1.6cm,
                minimum height=0.6cm, font=\scriptsize},
  arr/.style={-{Stealth[length=2mm]}, thick},
  level/.style={font=\scriptsize\itshape, text=gray}
]

% Chunks (leaves)
\node[chunk] (c0) {Chunk 0};
\node[chunk, right=0.3cm of c0] (c1) {Chunk 1};
\node[chunk, right=0.3cm of c1] (c2) {Chunk 2};
\node[chunk, right=0.3cm of c2] (c3) {Chunk 3};
\node[chunk, right=0.3cm of c3] (c4) {Chunk 4};
\node[chunk, right=0.3cm of c4] (c5) {Chunk 5};
\node[chunk, right=0.3cm of c5] (c6) {Chunk 6};
\node[chunk, right=0.3cm of c6] (c7) {Chunk 7};

% Leaf hashes
\node[hash, above=0.5cm of c0] (h0) {H(C0)};
\node[hash, above=0.5cm of c1] (h1) {H(C1)};
\node[hash, above=0.5cm of c2] (h2) {H(C2)};
\node[hash, above=0.5cm of c3] (h3) {H(C3)};
\node[hash, above=0.5cm of c4] (h4) {H(C4)};
\node[hash, above=0.5cm of c5] (h5) {H(C5)};
\node[hash, above=0.5cm of c6] (h6) {H(C6)};
\node[hash, above=0.5cm of c7] (h7) {H(C7)};

% Level 1
\node[hash, fill=orange!15, above=0.5cm of $(h0)!0.5!(h1)$] (n01) {H(01)};
\node[hash, fill=orange!15, above=0.5cm of $(h2)!0.5!(h3)$] (n23) {H(23)};
\node[hash, fill=orange!15, above=0.5cm of $(h4)!0.5!(h5)$] (n45) {H(45)};
\node[hash, fill=orange!15, above=0.5cm of $(h6)!0.5!(h7)$] (n67) {H(67)};

% Level 2
\node[hash, fill=yellow!20, above=0.5cm of $(n01)!0.5!(n23)$] (n03) {H(0--3)};
\node[hash, fill=yellow!20, above=0.5cm of $(n45)!0.5!(n67)$] (n47) {H(4--7)};

% Root
\node[hash, fill=red!15, above=0.5cm of $(n03)!0.5!(n47)$] (root) {Root};

% Edges
\foreach \c/\h in {c0/h0, c1/h1, c2/h2, c3/h3, c4/h4, c5/h5, c6/h6, c7/h7}
  \draw[arr, gray] (\c) -- (\h);
\draw[arr] (h0) -- (n01); \draw[arr] (h1) -- (n01);
\draw[arr] (h2) -- (n23); \draw[arr] (h3) -- (n23);
\draw[arr] (h4) -- (n45); \draw[arr] (h5) -- (n45);
\draw[arr] (h6) -- (n67); \draw[arr] (h7) -- (n67);
\draw[arr] (n01) -- (n03); \draw[arr] (n23) -- (n03);
\draw[arr] (n45) -- (n47); \draw[arr] (n67) -- (n47);
\draw[arr] (n03) -- (root); \draw[arr] (n47) -- (root);

% Proof path highlight
\draw[very thick, red, dashed, rounded corners]
  (h2.north west) +(-0.1,0.1) rectangle (h2.south east |- c2.south);
\draw[very thick, red, dashed]
  (h3.north west) +(-0.05,0.05) rectangle (h3.south east);
\draw[very thick, red, dashed]
  (n01.north west) +(-0.05,0.05) rectangle (n01.south east);
\draw[very thick, red, dashed]
  (n47.north west) +(-0.05,0.05) rectangle (n47.south east);

% Labels
\node[level, left=0.1cm of h0] {Leaf hashes};
\node[level, left=0.1cm of n01] {Level 1};
\node[level, left=0.1cm of n03] {Level 2};
\node[level, left=0.1cm of root] {Root};

% Annotation
\node[below=0.3cm of c3, font=\scriptsize, text=red] {%
  Verification path for Chunk~2: \{H(C3), H(01), H(4--7)\}};

\end{tikzpicture}
\caption{Merkle tree over an 8-chunk dataset. Red dashed boxes show the
$O(\log N)$ sibling hashes needed to verify Chunk~2 without reading any other
chunk data.}
\label{fig:merkle}
\end{figure}
