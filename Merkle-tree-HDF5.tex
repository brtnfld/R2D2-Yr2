...
program~\cite{shines}. Beyond the systems architecture (hybrid root-attribute
plus chunked companion dataset, Encrypt-then-MAC filter pipeline ordering,
version-counter nonce derivation for safe in-place updates), the work targets
three research contributions intended to be evaluated in the resulting
publication: (i)~a \emph{verifiable subset extraction} protocol enabling a
recipient of an arbitrary hyperslab to cryptographically verify it as a true
subset of a signed parent dataset; (ii)~a \emph{post-quantum hybrid signing}
scheme (Ed25519~$+$~ML-DSA-65) measured at HDF5 scale, addressing the 50-year
archival timescales typical of scientific data; and (iii)~a \emph{symbolic
security analysis} (Tamarin or game-based) of the EtM~$+$~version-counter~$+$
signed-root construction, evaluated against a directed-adversary red-team study
on real scientific datasets. The RustyHDF5 pure-Rust HDF5
...
\begin{itemize}[nosep]
  ...
  \item \textbf{KangarooTwelve}~\cite{k12}: A SHA-3-family parallel hash
    derived from \texttt{Keccak-p}, included to give FIPS-required deployments
    a parallel option without the SHA-256 throughput ceiling. Its 12-round
...
data to determine the performance--compliance tradeoff (SHA-256 for legacy
FIPS environments, KangarooTwelve for FIPS-aligned parallel deployments,
BLAKE3 for maximum throughput where compliance is unconstrained).
...
omitted, substituted from a different version, or reordered.
...
The proof~$\pi$ has three components: (a)~the union of Merkle witnesses for
the chunks in~$\mathcal{C}$, with shared internal nodes deduplicated;
(b)~a \emph{coverage certificate} consisting of the dataset's serialized
object header (datatype, dataspace dimensionality, chunk grid parameters,
and relevant attributes) together with its Merkle path to the file-level
signed root, allowing the recipient to independently verify the trusted grid
parameters and then recompute which chunks \emph{should} have been delivered
for hyperslab~$H$; and (c)~the per-chunk version counters, which are bound
into each leaf hash and therefore covered by the witnesses.
...
not its volume, yielding $O\bigl(\sum_i m_i \cdot \log n_i\bigr)$
for axis-aligned hyperslabs under an ideal tree layout. For
strided or irregular access patterns (e.g., every 10th time step),
deduplication savings are smaller and depend on the companion dataset's
linearization order. The optimal layout and the achievable compression
ratio across realistic HPC access patterns is a research question we
intend to characterize empirically (RQ6, \S\ref{sec:research-questions}).
...
and \texttt{0x01} before hashing internal node pairs. This makes leaf and
internal hashes structurally incompatible at the input level, regardless of
...
\paragraph{Post-quantum hybrid signing (T5: harvest-now, decrypt-later).}
\label{sec:pq-hybrid}
...
\begin{equation}
  ...
  \quad R = \text{root} \,\|\, \text{companion\_hash} \,\|\, v_\text{dataset} \,\|\, \tau
\end{equation}

\noindent where $R$ is the canonical signed payload (root, companion integrity
hash, dataset-level version counter, and timestamp). ML-DSA-65 (NIST FIPS
204~\cite{fips204}) provides $\sim$192-bit classical and $\sim$128-bit
post-quantum security; SLH-DSA-128s (NIST FIPS 205~\cite{fips205}) is also
...
verifier accepts the signature iff \emph{both} components verify, providing
graceful degradation: until a CRQC exists, Ed25519 dominates verification
time; once one exists, the ML-DSA-65 component preserves authenticity.
...
$10^6$--$10^9$ chunks, is the substance of RQ7
...
\begin{description}[nosep]
  \item[Game-based argument (baseline, in scope).]
    ...
    signed root over companion-integrity hash)} achieves INT-CTXT against
    ...
    (c)~the IND-CPA security of the chosen AEAD cipher. We follow the
    Bellare--Namprempre framework for analyzing generic
...
\begin{enumerate}[label=\textbf{RQ\arabic*}]
  ...
  \item \textbf{(Post-quantum signing.)}~What are the storage, sign, and
    ...
    signed roots at $10^4$--$10^5$ datasets per file and $10^6$--$10^9$
    chunks per dataset, including object-header impact and end-to-end
    verification time on x86-64 (with and without AVX-512) and ARM?
  \item \textbf{(Adversarial soundness.)}~Can a directed adversary with
    knowledge of the design and full read/write access to the storage
    medium evade detection by the verification protocol within the stated
    threat model (T1--T5)? This is evaluated via the red-team study
    described in~\S\ref{sec:adversarial}.
\end{enumerate}
...
(Move~1), post-quantum hybrid signing (Move~2), and a security analysis
backed by an adversarial red-team study. Items not on the critical path to
these contributions---in particular AES-256-CTR~$+$~HMAC-SHA-256 filters and
live Sigstore service integration---are deferred to Phase~3 / future work to
preserve focus.
...
\begin{enumerate}[nosep]
  \item \textbf{Hybrid signing (Move~2):} implement
    ...
    \,\|\, \tau$ (\S\ref{sec:pq-hybrid}), with Ed25519~$+$~SLH-DSA-128s
    available as an alternative. Measure end-to-end sign/verify cost,
    signature and public-key size, and object-header impact at
    $10^4$--$10^5$ datasets per file and $10^6$--$10^9$ chunks per dataset
    (RQ7) on x86-64 (with and without AVX-512) and ARM.
  ...
  \item \textbf{Adversarial red-team study (Move~3):} execute the white-box
    ...
    covering T1--T5 plus the subset-extraction attack classes. Report the
  ...
  \item \textbf{Cloud object-store evaluation (Move~6):} implement the
    ...
    cloud read paths in RustyHDF5) and run the intra-region and
...
\begin{enumerate}[nosep, resume]
  \item \textbf{Append-only log Merkle mode}: a
...
\midrule
Merkle-tree provenance implementation in RustyHDF5 (hybrid storage,
...
Game-based security argument for EtM~$+$~version-counter~$+$~hybrid-signed-root
  (Move~4)
  & Task 3 & 2 & Core \\
$\star$~Adversarial red-team study on NOAA and EQSIM datasets (T1--T5
...
\begin{itemize}[nosep]
  \item \textbf{SHINES alignment}: The Merkle-tree prototype implements
    ...
    current vocabulary.
  \item \textbf{Task 2 connection}: Ed25519~$+$~ML-DSA-65 hybrid root signing
    uses the same keychain infrastructure being developed for plugin digital
    signatures, while introducing the post-quantum hybrid pattern that Task~2
    will need for plugin signing on the same archival timescales. As a
    stretch goal, Sigstore transparency-log publication would further unify
    the trust framework across S2-D2 thrusts.
...
\begin{description}[nosep]
  ...
  \item[Adversarial-study scope.] A white-box red-team study can in principle
    ...
    \S\ref{sec:threat} (T1--T5) plus the three subset-extraction attack
...
$\rightarrow$ Merkle trees are the right primitive
...
The game-based INT-CTXT argument and adversarial red-team study are collected in
...
HDF5. Cites Peisert~2021. States three contributions up front. RustyHDF5 rationale:
...
\bigskip
%% §2
\noindent\textcolor{blue!60!black}{\textbf{\S2\quad Background \& Related Work \hfill($\sim$1.5\,pp)}}
\par\noindent\rule{\linewidth}{0.4pt}
\mapsec{Proposal \S2}{---}{---}

\noindent\textbf{Content:} HDF5 chunked storage, filter pipeline, RustyHDF5
architecture. Related work: Merkle trees in storage (ZFS, Git, IPFS, Certificate
Transparency), scientific format integrity (NetCDF, Zarr, ASDF), authenticated data
structures (Tamassia), provenance (W3C PROV), PQ signatures (FIPS~204/205). Brief note
on Tang et al.\ async I/O as future STS context.

\noindent\textbf{Writing strategy:} Sharpen differentiation --- what does this do that
ZFS/IPFS/Git/Zarr-v3 don't? Answer: chunk-aligned trees with HDF5 hyperslab-aware
partial verification, verifiable subset extraction, and PQ-ready signed roots. Target
20$+$ references.
...
\par\noindent\rule{\linewidth}{0.4pt}
\mapsec{Proposal tasks 1.1--1.7, 2.1, 2.2, 2.4, 2.5 (Phases~1--2)}{RQ1, RQ2, RQ3,
...
(4.6)~AEAD integration for ChaCha20-Poly1305.
(4.7)~Crash consistency --- write ordering, fail-closed for signed datasets.
(4.8)~Key management --- DEK/KEK/HKDF baseline.
...
\par\noindent\rule{\linewidth}{0.4pt}
\mapsec{Proposal tasks 2.3, 2.4 (Phase~2); Tamarin model (Phase~3 stretch)}{RQ8}{T1--T5}
...
(5.2)~$\star$~Adversarial red-team study: white-box attack suite covering T1--T5 plus
...
\par\noindent\rule{\linewidth}{0.4pt}
\mapsec{Proposal tasks 1.8, 2.5, 2.6 (Phases~1--2)}{---}{---}
...
\par\noindent\rule{\linewidth}{0.4pt}
\mapsec{Proposal tasks 1.7, 1.8, 2.4, 2.5, 2.6}{RQ1--RQ8}{---}
...
\par\noindent\rule{\linewidth}{0.4pt}
\mapsec{Proposal task 2.6 (design document)}{---}{---}
...
single-platform scope, RustyHDF5-specific behaviors. Future work: C~HDF5 integration,
...
\textbf{Task} & \textbf{Description} &
  ...
  & & Design & Security & Results & Discuss \\
\midrule
1.1  & MerkleTree struct + hash eval (SHA-256, BLAKE3, K12) & \checkmark & & \checkmark~(RQ4) & \\
1.2  & Hybrid storage layout + companion integrity hash      & \checkmark & & \checkmark~(RQ1) & \\
1.3  & \texttt{verify\_root()}                              & \checkmark & & \checkmark~(RQ1) & \\
1.4  & \texttt{verify\_chunk()}                             & \checkmark & & \checkmark~(RQ2) & \\
1.5  & \texttt{verify\_dataset()}                           & \checkmark & & \checkmark~(RQ5) & \\
1.6  & \texttt{extend\_merkle} + \texttt{update\_merkle}   & \checkmark & & \checkmark~(RQ3) & \\
1.7\,$\star$ & Verifiable subset extraction protocol        & \checkmark & & \checkmark~(RQ6) & \\
1.8  & Phase~1 benchmarks                                   & & & \checkmark~(RQ1--4) & \\
\midrule
2.1\,$\star$ & Hybrid Ed25519\,$+$\,ML-DSA-65 signing      & \checkmark & & \checkmark~(RQ7) & \\
2.2  & ChaCha20-Poly1305 AEAD + nonce derivation            & \checkmark & & & \\
2.3  & Game-based security argument                         & & \checkmark & & \\
2.4\,$\star$ & Adversarial red-team study                   & & \checkmark & \checkmark~(RQ8) & \\
2.5  & Cloud / WAN evaluation                               & & & \checkmark~(RQ6) & \\
2.6  & C~HDF5 design document + RFC                         & & & & \checkmark \\
\midrule
2.7* & Append-only log mode                                 & \checkmark & & & \checkmark \\
2.8* & Tamarin symbolic model                               & & \checkmark & & \\
2.9* & Sigstore + Rekor                                     & \checkmark & & & \\
\bottomrule
...
\begin{itemize}[nosep]
  \item Table~1: Merkle tree storage overhead for representative dataset sizes
    (existing) --- \S4
  \item Table~2: Threat model --- mechanism $\rightarrow$ threat mapping (T1--T5) --- \S3
...
\midrule
Month~4 (after G2) & \S1--\S4 &
  Draft introduction, background, threat model, Merkle design. \\
  & & Stable after Phase~1 go/no-go. Circulate for feedback. \\
Month~5--6 & \S5, \S6, \S7 &
  Security analysis; experimental setup (after benchmark harness). \\
  & & Write results subsection-by-subsection as RQ data arrives. \\
Month~7 & \S8, \S9, \S1 rev &
  Discussion + conclusion. Revise \S1 based on actual results. \\
  & & Final consistency pass. Internal review before submission. \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[nosep]
  \item \textbf{CCGrid / IPDPS (12\,pp):} Primary target at current scope. Comfortable
    page budget for 8 RQs. Aligned with S2-D2 team publications.
  \item \textbf{PDSW (SC workshop, 10\,pp):} Natural fit for HPC data provenance.
    Achievable by compressing PQ signing and verifiable subset to one paragraph each
    and dropping the cloud / WAN evaluation. Deadline typically August for November SC.
