...
(b)~a \emph{coverage certificate} binding the dataset's chunk-grid parameters
(stored in the dataset's object header and covered by the file-level root) to
the requested hyperslab~$H$, so the verifier can recompute which chunks
\emph{should} have been delivered; and (c)~the per-chunk version counters,
which are bound into each leaf hash and therefore covered by the witnesses.
...
gives $|\pi| = O\bigl(\prod_i m_i \cdot \log \prod_i n_i\bigr)$. For
strided or sparse hyperslabs typical of scientific access (e.g., every 10th
time step), this can be substantially compressed by exploiting shared
internal nodes; the optimal layout for the companion dataset that minimizes
expected proof size for realistic access patterns is an open question we
intend to investigate empirically (RQ6, \S\ref{sec:research-questions}).
...
\paragraph{Why this needs to be measured at HDF5 scale.}
...
to $\sim$30~KB depending on parameters). At the dataset level this is
negligible, but a \emph{file-level} signed root that aggregates roots from
$10^4$--$10^5$ datasets in a single HDF5 file (typical of HPC checkpoint
files) interacts with HDF5's object-header encoding rules in ways that have
not been measured. Likewise, batch verification throughput at $10^6$--$10^9$
chunks under per-dataset verification is unknown for any PQ scheme on HDF5
workloads. Producing this measurement is RQ7
...
\begin{description}[nosep]
  ...
  \item[Symbolic model (stretch, preferred for security venues).]
    ...
    (authenticity, freshness, subset soundness). The Tamarin model would
    be released alongside the implementation.
\end{description}
