...
confidentiality at the cost of excluding third-party auditors; SKderived
...
\end{lstlisting}
  \item Implement \texttt{MerkleTree::build(leaves: \&[[u8;32]], alg: HashAlg)
    ...
    power-of-2 chunk counts. Pad the leaf array with \texttt{0x02} null
    hashes (as defined in \S\ref{sec:merkle-storage}) up to the next power
...
\end{lstlisting}
  ...
  \item Implement version-counter nonce derivation. The nonce for encrypting
    ...
    \texttt{u32} in the companion dataset alongside each leaf hash.
