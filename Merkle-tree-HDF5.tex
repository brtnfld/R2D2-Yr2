...
all listed codes are linear over $\mathrm{GF}(2)$ (the binary field
$\{0,1\}$ under XOR arithmetic) and can be inverted in
closed form by an adversary: given a desired alteration, the attacker uses
elementary linear algebra over the binary field to compute
a matching tail correction that preserves the checksum---no brute-force search is required. They therefore detect
...
(${\sim}200$ cores $\times$ 1.5\,GB/s $\approx$ 300\,GB/s aggregate for 10\,TB);
...
\begin{itemize}[nosep]
  ...
  \item Build cost (R10) is one hash per chunk plus $N-1$ pair hashes for
    ...
    $\sim$30\,s on a typical HPC node (parallel I/O across ${\sim}200$ cores
    at 1.5\,GB/s); tree aggregation over pre-computed leaf hashes adds
    $<1$\,s on a single core---two or more orders of magnitude below the
